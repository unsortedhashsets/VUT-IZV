...
def plot_stat(data_source, fig_location = None, fig_name ='noname.png', show_figure = False):
    ...
    for i, year in enumerate(years):
        ...
        for region in regions:
            marks.append(np.count_nonzero(np.logical_and(data_source[1][0] == region, data_source[1][4].astype('datetime64[Y]') == year)))

        if today.astype('datetime64[Y]') == year or today.astype('datetime64[M]') == np.datetime64(f'{year+1}-01'):
            for region in regions:
                average_marks.append(np.count_nonzero(np.logical_and(data_source[1][0] == region, data_source[1][4].astype('datetime64[Y]') != year))/(len(years)-1))
    ...
    print(f"--- {(time.time() - start_time)/70}")
